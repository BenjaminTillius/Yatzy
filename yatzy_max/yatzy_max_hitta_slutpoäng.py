...
förberäknar policy, och letar efter ett spel där totalpoängen är exakt 306.
"""
import numpy as np
...
# ── Leta efter spel med totalpoäng 306 ───────────────────────────────────────
print(f"\nLetar efter spel med totalpoäng {TARGET_SCORE}...")
...
for sim in range(10_000_000):
    ...
    if total + bonus == TARGET_SCORE:
        ...
        print(f"Totalpoäng: {total + bonus}")
        print(f"\n{'Runda':<6} {'Kast 1':<20} {'Behåll':<20} {'Kast 2':<20} {'Behåll':<20} {'Kast 3':<20} {'Kategori':<14} {'Poäng':<6} {'Totalt före'}")
        print("-" * 150)
        for h in history:
            print(f"{h['runda']:<6} {str(h['kast1']):<20} {str(h['behåll1']):<20} "
                  f"{str(h['kast2']):<20} {str(h['behåll2']):<20} {str(h['kast3']):<20} "
                  f"{h['kategori']:<14} {h['poäng']:<6} {h['total_före']}")
        break
else:
    print(f"Hittade inget spel med totalpoäng {TARGET_SCORE} på 10 000 000 försök.")