"""
Beräknar medelvärdet av det bästa resultatet på 4 spel med optimal strategi.
Simulerar 100 000 omgångar om 4 spel och sparar det högsta per omgång.
"""
import numpy as np
...
# ── Simulering: bästa av 4 spel ──────────────────────────────────────────────
def play_game(rng, mask, gamma):
    total = 0
...
print(f"Simulerar {N_ROUNDS} omgångar om {GAMES_PER_ROUND} spel...")
rng     = np.random.default_rng()
best_of = np.zeros(N_ROUNDS, dtype=np.int32)

for rnd in range(N_ROUNDS):
    best_of[rnd] = max(play_game(rng, 0, GAMMA_MAX) for _ in range(GAMES_PER_ROUND))

print(f"\nBästa av {GAMES_PER_ROUND} spel ({N_ROUNDS} omgångar):")
print(f"  Medelvärde:       {best_of.mean():.4f}")
print(f"  Median:           {np.median(best_of):.4f}")
print(f"  Standardavvikelse:{best_of.std():.4f}")
print(f"  Min / Max:        {best_of.min()} / {best_of.max()}")