"""
Rekordspel för exempelspelet i uppsatsen.
  k=2 (tvåsidiga tärningar), n=2 (två tärningar),
  h=2 (kategorier: ettor och tvåor), rho_max=2 (två slag), beta=5.

Kategorier:
  0: Ettor — poäng = antal ettor * 1
  1: Tvåor — poäng = antal tvåor * 2

Belöningsfunktioner (rekordspel):
...
round_start = np.zeros((N_MASKS, NT, NU))
round_start[FULL, BETA, 0] = 1.0
...
for mask in range(FULL - 1, -1, -1):
    ...
    if not open_cats:
        continue
    V0 = np.zeros((NT, NU, ND))
    for cat in open_cats:
        nm = mask | (1 << cat)
        nt = new_tau[cat]
        rs = round_start[nm][nt[:, None, :],
                             np.zeros((NT, 1, ND), dtype=int)]
        np.maximum(V0, rs, out=V0)
    V1  = best_reroll_batch(V0)
    V1f = V1.reshape(NT * NU, ND)
    round_start[mask] = (T_start @ V1f.T).reshape(NT, NU)

print(f"u*(s_1) = {round_start[0, 0, 0]:.6f}")

# Simulering
cache = {}

def get_policy(mask, tau):
    key = (mask, tau)
    if key in cache:
        return cache[key]
    open_cats = [c for c in range(N_CATS) if not (mask >> c & 1)]
    V0 = np.zeros(ND); best_c = np.zeros(ND, dtype=np.int8)
    for cat in open_cats:
        nm = mask | (1 << cat)
        r  = score_table[:, cat]
        nt = np.minimum(tau + r, BETA).astype(int)
        rs = round_start[nm, nt, 0]
        improved = rs > V0
        V0[improved]     = rs[improved]
        best_c[improved] = cat
    ev1   = T @ V0
    keep1 = np.array([valid_ki[di][ev1[valid_ki[di]].argmax()] for di in range(ND)], dtype=np.int16)
    cache[key] = (best_c, keep1)
    return cache[key]
...
for sim in range(N_SIM):
    mask, tau, total = 0, 0, 0
    for _ in range(N_CATS):
        best_c, keep1 = get_policy(mask, tau)
